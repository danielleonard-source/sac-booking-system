...
function RWFormModal() {
  const { showRWForm, editingRW, closeRWForm, showNotification } = useUIStore();
  ...
  useEffect(() => {
    if (showRWForm && editingRW) {
      // Edit mode
      setForm({
        firstName: editingRW.firstName || '',
        lastName: editingRW.lastName || '',
        email: editingRW.email || '',
        daysAvailable: editingRW.daysAvailable || '',
        employmentType: editingRW.employmentType || 'casual' // NEW - default to casual if not set
      });
    } else if (showRWForm) {
      // New mode
      setForm({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingRW) {
        await updateReaderWriter(editingRW.id, form);
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-lg w-full">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 flex justify-between items-center">
          <h2 className="text-2xl font-bold text-white">
            {editingRW ? '✏️ Edit Reader/Writer' : '➕ Add New Reader/Writer'}
          </h2>
          <button
            onClick={closeRWForm}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t">
            ...
            >
              {editingRW ? 'Update Reader/Writer' : 'Add Reader/Writer'}
            </button>
          </div>
        </form>
      </div>
